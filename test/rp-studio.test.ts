...
const engineUrl = new URL("../apps/roleplay/plugins/engine/plugin.js", import.meta.url).href;
const stUrl = new URL("../apps/roleplay/plugins/studio-import/plugin.js", import.meta.url).href;

// gate: every shipped app's look scripts must TRANSFORM (a look file that
// doesn't parse ships a broken app — the UI test surface can't see it)
describe("shipped app look files transform", () => {
  const appsDir = fileURLToPath(new URL("../apps", import.meta.url));
  for (const app of ["roleplay"]) {
    it(`${app}: ships as a standard web app, with nothing left of the retired look tier`, () => {
      expect(fs.existsSync(path.join(appsDir, app, "src", "main.tsx"))).toBe(true);
      // built in the browser: index.html + src/, and no bundler config to run
      expect(fs.existsSync(path.join(appsDir, app, "index.html"))).toBe(true);
      expect(fs.existsSync(path.join(appsDir, app, "vite.config.ts"))).toBe(false);
      expect(fs.existsSync(path.join(appsDir, app, "index.html"))).toBe(true);
      expect(fs.existsSync(path.join(appsDir, app, "look"))).toBe(false);
      const m = JSON.parse(fs.readFileSync(path.join(appsDir, app, "manifest.json"), "utf8")) as Record<string, unknown>;
