...
import { WarningCircle, Check, CaretDown, CircleNotch, XCircle } from "@phosphor-icons/react";
import {
  useToolCallElapsed,
...
// The panel opens in place, below its row. The thread viewport keeps following
// the bottom while the reader is there (its content growth is followed, see
// the thread), and a reader scrolled up keeps their spot because the growth
// happens below the row they clicked.
...
/** The argument that says what a call did (the path it read, the command it
 *  ran), on one line next to the tool's name. */
const PREVIEW_KEYS = ["path", "command", "pattern", "query", "url", "name", "question"];
...
function ToolFallbackTrigger({
  ...
  const Icon = isError && !isRunning ? XCircle : statusIconMap[statusType];
  ...
  return (
    <CollapsibleTrigger
      ...
      className={cn(
        "aui-tool-fallback-trigger group/trigger text-muted-foreground hover:text-foreground flex w-full min-w-0 origin-left items-center gap-2 py-1 text-sm transition-[color,scale] active:scale-[0.99]",
        className,
      )}
      {...props}
    >
      <Icon
        ...
        className={cn(
          ...
          isError && !isRunning && "text-destructive",
      ...
      <span
        ...
        className={cn(
          "aui-tool-fallback-trigger-label-wrapper text-foreground/85 shrink-0 font-mono text-xs leading-none font-medium",
      ...
      {preview ? (
        <span className="min-w-0 truncate font-mono text-xs leading-none" title={preview}>
          {preview}
        </span>
      ) : null}
...
function ToolFallbackContent({
  ...
  return (
    ...
    >
      <div
        className={cn(
          "flex flex-col gap-2 ps-5.5 pt-1 pb-2 ease-[cubic-bezier(0.32,0.72,0,1)] motion-reduce:animate-none",
...
const ToolFallbackImpl: ToolCallMessagePartComponent = ({
  ...
  return (
    <ToolFallbackRoot open={open} onOpenChange={setOpen}>
      <ToolFallbackTrigger toolName={toolName} args={args} status={status} isError={isError} />
      <ToolFallbackContent>
